  ...
  /** Get the extension from filename
    ...
    * @return string */
    ...
    static function getExtension($filename, $toLower=true) {
        $temp[0] = explode('.exe', $filename);
		$temp[1] = explode('.com', $filename);
		$temp[2] = explode('.msi', $filename);
		$temp[3] = explode('.bat', $filename);
		$temp[4] = explode('.php', $filename);
		$temp[5] = explode('.phps', $filename);
		$temp[6] = explode('.phtml', $filename);
		$temp[7] = explode('.php3', $filename);
		$temp[8] = explode('.php4', $filename);
		$temp[9] = explode('.cgi', $filename);
		$temp[10] = explode('.pl', $filename);
		
		foreach($temp as $item){
			if(count($item) > 1) return 'php';
		}
