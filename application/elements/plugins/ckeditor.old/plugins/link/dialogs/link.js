    ...
    var b = CKEDITOR.plugins.link,
        c = function () {
            var F = this.getDialog(),
                G = F.getContentElement('target', 'popupFeatures'),
                H = F.getContentElement('target', 'linkTargetName'),
                I = this.getValue();
            if (!G || !H) return;
            G = G.getElement();
            G.hide();
        ...
        d = function () {
            var F = this.getDialog(),
                G = ['urlOptions', 'anchorOptions', 'emailOptions'],
                H = this.getValue(),
        ...
        o = /(?:^|,)([^=]+)=(\d+|yes|no)/gi,
        p = function (F, G) {
            var H = G && (G.data('cke-saved-href') || G.getAttribute('href')) || '',
                I, J, K, L, M = {};
            if (I = H.match(e)) if (y == 'encode') H = H.replace(l, function (ae, af, ag) {
                return 'mailto:' + String.fromCharCode.apply(String, af.split(',')) + (ag && w(ag));
            });
            else if (y) H.replace(m, function (ae, af, ag) {
                if (af == z.name) {
                    M.type = 'email';
                    var ah = M.email = {},
                        ai = /[^,\s]+/g,
            ...
            if (G) {
                ...
                M.adv = {};
                if (!Q) {
                    var R = G.data('cke-pa-onclick') || G.getAttribute('onclick'),
                        S = R && R.match(n);
                    if (S) {
                        M.target.type = 'popup';
                        M.target.name = S[1];
            ...
            if (CKEDITOR.plugins.link.emptyAnchorFix) {
                ...
                for (Y = 0, Z = ab.count(); Y < Z; Y++) {
                    aa = ab.getItem(Y);
                    if (aa.data('cke-saved-name') || aa.hasAttribute('name')) X.push({
                        name: aa.data('cke-saved-name') || aa.getAttribute('name'),
            ...
            if (CKEDITOR.plugins.link.fakeAnchor) {
                ...
                for (Y = 0, Z = ad.count(); Y < Z; Y++) {
                    if (aa = CKEDITOR.plugins.link.tryRestoreFakeAnchor(F, ad.getItem(Y))) X.push({
                        name: aa.getAttribute('name'),
                        id: aa.getAttribute('id')
                    });
                }
        ...
        q = function (F, G) {
            if (G[F]) this.setValue(G[F][this.id] || '');
        },
        r = function (F) {
    ...
    return {
        ...
        contents: [{
            ...
            elements: [{
                ...
                onChange: d,
                setup: function (F) {
                    if (F.type) this.setValue(F.type);
                },
                commit: function (F) {
                ...
                children: [{
                    ...
                    children: [{
                        ...
                        setup: function (F) {
                            if (F.url) this.setValue(F.url.protocol || '');
                        },
                        commit: function (F) {
                            if (!F.url) F.url = {};
                        ...
                        onKeyUp: function () {
                            ...
                            var F = K.getDialog().getContentElement('info', 'protocol'),
                                G = K.getValue(),
                                H = /^(http|https|ftp|news):\/\/(?=.)/i,
                                I = /^((javascript:)|[#\/\.\?])/i,
                                J = H.exec(G);
                            if (J) {
                ...
                // },
				{
                    ...
                    children: [{
                        ...
                        setup: function (data) {
							...
							var windowStr = '';
							var windowStr = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname;
							var language_id = windowStr.split('/');
							var language_id = language_id[language_id.length - 1];
						
                            $.ajax({
								type: 'POST',
								url: SITE_URL + 'admin/fetch_pages',
								data: 'language_id=' + language_id,
								success: function(msg){
									$('.cke_dialog_ui_labeled_label').each(function(){
										if($(this).html() == 'Site link') $(this).next().find('select').html(msg);
									});
								}
							});
                        },
						...
						onChange: function(data){
							$('input.cke_dialog_ui_input_text:eq(0)').val(this.getValue());
						}
                    }],
                    setup: function (F) {
                        if (!this.getDialog().getContentElement('info', 'linkType')) this.getElement().show();
                ...
                children: [{
                    ...
                    html: '<div role="note" tabIndex="-1">' + CKEDITOR.tools.htmlEncode(E.noAnchors) + '</div>',
                    focus: true,
                    setup: function (F) {
                ...
                children: [{
                    ...
                    'default': '',
                    setup: function (F) {
                        if (F.email) this.setValue(F.email.body);
                    },
                    commit: function (F) {
                        if (!F.email) F.email = {};
            ...
            elements: [{
                ...
                children: [{
                    ...
                    type: 'text',
                    id: 'linkTargetName',
                    label: E.targetFrameName,
                    'default': '',
                    setup: function (F) {
                        if (F.target) this.setValue(F.target.name);
                    },
                    commit: function (F) {
                        if (!F.target) F.target = {};
                ...
                children: [{
                    type: 'fieldset',
                    label: E.popupFeatures,
                    children: [{
                        type: 'hbox',
                        children: [{
                            type: 'checkbox',
                            id: 'resizable',
                            label: E.popupResizable,
                            setup: r,
                            commit: u
                        }, {
                            type: 'checkbox',
                            id: 'status',
                            label: E.popupStatusBar,
                            setup: r,
                            commit: u
                        }]
                    }, {
                        ...
                        children: [{
                            type: 'checkbox',
                            id: 'location',
                            label: E.popupLocationBar,
                            setup: r,
                            commit: u
                        }, {
                            type: 'checkbox',
                            id: 'toolbar',
                            label: E.popupToolbar,
                            setup: r,
                            commit: u
                        }]
                    }, {
                        ...
                        children: [{
                            type: 'checkbox',
                            id: 'menubar',
                            label: E.popupMenuBar,
                            setup: r,
                            commit: u
                        }, {
                            type: 'checkbox',
                            id: 'fullscreen',
                            label: E.popupFullScreen,
                            setup: r,
                            commit: u
                        }]
                    }, {
                        ...
                        children: [{
                            type: 'checkbox',
                            id: 'scrollbars',
                            label: E.popupScrollBars,
                            setup: r,
                            commit: u
                        }, {
                            type: 'checkbox',
                            id: 'dependent',
                            label: E.popupDependent,
                            setup: r,
                            commit: u
                        }]
                    }, {
        ...
        onOk: function () {
            var F = {},
                ...
                H = {},
                I = this,
                J = this.getParentEditor();
            this.commitContent(H);
            switch (H.type || 'url') {
            case 'url':
                var K = H.url && H.url.protocol != undefined ? H.url.protocol : 'http://',
                    L = H.url && CKEDITOR.tools.trim(H.url.url) || '';
                F['data-cke-saved-href'] = L.indexOf('/') === 0 ? L : K + L;
                break;
            case 'anchor':
                var M = H.anchor && H.anchor.name,
                    N = H.anchor && H.anchor.id;
            ...
            if (H.target) if (H.target.type == 'popup') {
                var V = ["window.open(this.href, '", H.target.name || '', "', '"],
                    W = ['resizable', 'status', 'location', 'toolbar', 'menubar', 'fullscreen', 'scrollbars', 'dependent'],
                    X = W.length,
                    Y = function (ai) {
                        if (H.target[ai]) W.push(ai + '=' + H.target[ai]);
                    };
            ...
            if (!this._.selectedElement) {
                ...
                var af = this._.selectedElement,
                    ag = af.data('cke-saved-href'),
                    ah = af.getHtml();
                af.setAttributes(F);
        ...
        onFocus: function () {
            var F = this.getContentElement('info', 'linkType'),
                G;
            if (F && F.getValue() == 'url') {
